...
#' #' X_on_Z_fam <- "binomial"
#' #' Y_on_Z_fam <- "poisson"
#' #' results <- GCM_v2(data, X_on_Z_fam, Y_on_Z_fam)
#' #' results$test_stat
#' #' results$p_value
...
#' #'
#' #' \code{dCRT_v2} is a function carrying out the dCRT based on GLMs for `X|Z` and `Y|Z`.
#' #'
#' #' @param data A named list with fields \code{X} (an nx1 vector for the predictor
#' #' variable of interest), \code{Y} (an nx1 response vector), and \code{Z}
#' #' (an nxp matrix of covariates).
...
#' #' X_on_Z_fam <- "binomial"
#' #' Y_on_Z_fam <- "poisson"
#' #' results <- dCRT_v2(data, X_on_Z_fam, Y_on_Z_fam, B, normalize, return_resamples)
#' #' results$test_stat
#' #' results$p_value
...
#' #'
#' #' \code{spaCRT_v2} is a function carrying out the saddlepoint approximation to the
...
#' #' X_on_Z_fam <- "binomial"
#' #' Y_on_Z_fam <- "poisson"
#' #' results <- spaCRT_v2(data, X_on_Z_fam, Y_on_Z_fam, normalize)
#' #' results$test_stat
#' #' results$p_value
#' #'
#' #' @return A named list with fields \code{test_stat}, \code{left_side_p_value},
...
#' #' return_cdf == TRUE.
#' #' gcm.default returns TRUE if spacrt::GCM_v2 was employed due to the failure of spaCRT.
#' #'
...
#'     temp.gcm <- spacrt::GCM_v2(data, X_on_Z_fam, Y_on_Z_fam,
...
#' #' Score Test
#' #'
#' #' \code{score.test_v2} is a function carrying out the saddlepoint approximation to the
#' #' dCRT based on GLMs for `X|Z` and `Y|Z`.
#' #'
#' #' @param data A named list with fields \code{X} (an nx1 vector for the predictor
#' #' variable of interest), \code{Y} (an nx1 response vector), and \code{Z}
...
#' #' X_on_Z_fam <- "binomial"
#' #' Y_on_Z_fam <- "poisson"
#' #' results <- score.test_v2(data, X_on_Z_fam, Y_on_Z_fam)
#' #' results$test_stat
#' #' results$p_value
#' #'
#' #' @return A named list with fields \code{test_stat} and \code{left_side_p_value},
...
#' #'
#' score.test_v2 <- function(data, X_on_Z_fam = NULL, Y_on_Z_fam = NULL,
#'                        fit_glm_X = TRUE, fit_glm_Y = TRUE,
#'                        glm.nb.default = TRUE,
#'                        aux_info_X_on_Z = NULL, aux_info_Y_on_Z = NULL){
#'
...
#'                                     mustart = aux_info_Y_on_Z$fitted_values))
#'         })
#'       }else if(!is.null(aux_info_Y_on_Z)){
#'         Y_on_Z_fit <- suppressWarnings(stats::glm(Y ~ Z,
#'                                   family = MASS::negative.binomial(aux_info_Y_on_Z$theta_hat),
#'                                   mustart = aux_info_Y_on_Z$fitted_values))
#'       }else if(is.null(aux_info_Y_on_Z)){
...
#'                                   mustart = aux_info_Y_on_Z$fitted_values))
#'
#'         warning("NB dispersion parameter was estimated through pre-computation.")
#'       }
